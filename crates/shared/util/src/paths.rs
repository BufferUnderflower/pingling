...
impl RuntimePaths {
    ...
    pub fn current() -> Self {
        let config_root = env_path("PINGLING_CONFIG_ROOT").unwrap_or_else(|| {
            app_root(dirs::config_dir().unwrap_or_else(|| std::env::temp_dir()))
        });
        let cache_root = env_path("PINGLING_CACHE_ROOT")
            .unwrap_or_else(|| app_root(dirs::cache_dir().unwrap_or_else(|| std::env::temp_dir())));
        Self {
            ...
            active_config_temp_dir: std::env::temp_dir().join("pingling-active-configs"),
            config_root,
            cache_root,
        }
    }
...
mod tests {
    ...
    fn install_runtime_env(root: &std::path::Path) -> Vec<EnvGuard> {
        ...
        guards.push(EnvGuard::set(
            "PINGLING_CONFIG_ROOT",
            &expected_config_root(root),
        ));
        guards.push(EnvGuard::set(
            "PINGLING_CACHE_ROOT",
            &expected_cache_root(root),
        ));
        for key in [
            "HOME",
            "XDG_CONFIG_HOME",
    ...
    fn expected_config_root(root: &std::path::Path) -> std::path::PathBuf {
        #[cfg(target_os = "macos")]
        {
            root.join("Library/Application Support/pingling")
        }
    ...
    fn expected_cache_root(root: &std::path::Path) -> std::path::PathBuf {
        #[cfg(target_os = "macos")]
        {
            root.join("Library/Caches/pingling")
        }
    ...
    fn current_runtime_paths_respect_config_and_cache_roots() {
        ...
        assert_eq!(
            ...
            std::env::temp_dir().join("pingling-active-configs")
        );
    }

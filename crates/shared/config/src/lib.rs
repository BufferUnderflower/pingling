//! Generic multi-source configuration for a small Pingling host.
//!
...
//!
//! Downstream hosts choose their own app name, environment prefix, config
//! paths, and defaults. The built-in defaults are intentionally small enough
//! for examples and local experiments.
...
pub struct PinglingConfig {
    ...
    /// Legacy tray toggle kept for config compatibility.
...
impl ConfigLoader {
    ...
    pub fn with_system_paths(mut self, include: bool) -> Self {
        self.include_system_paths = include;
        self
    }
}
...
impl ConfigLoader {
    ...
    fn apply_env_overrides(&self, cfg: &mut PinglingConfig) {
    ...
    pub fn load_file(&self, path: &Path) -> Result<PinglingConfig, String> {
        let ext = path
            .extension()
            .and_then(|e| e.to_str())
            .unwrap_or("yaml")
    ...
    pub fn from_file(path: &Path) -> Result<PinglingConfig, String> {
        Self::default().load_file(path)
    }

    /// Load from default locations + env var overrides.
    ///
    /// Search order:
    /// - user config dir: `<config>/<app_name>/config.{yaml,json}`
    /// - Unix system dir, when enabled: `/etc/<app_name>/config.{yaml,json}`
    ///
    /// Falls back to defaults + env if no file found.
    pub fn load_env(&self) -> Result<PinglingConfig, String> {
        let default_paths = self.search_paths();
    ...
    pub fn load_env_only(&self) -> Result<PinglingConfig, String> {
    ...
    pub fn from_env_only() -> Result<PinglingConfig, String> {
        Self::default().load_env_only()
    }

    /// Returns the default config file search paths.
    ///
    /// The app name is configurable; the default app name is `pingling`.
...
"#,
    ...
    fn search_paths_are_non_empty() {
        let paths = ConfigLoader::new("demo-host")
            .with_system_paths(false)
            .search_paths();
        assert!(!paths.is_empty());
        assert!(paths
            .iter()
            .any(|p| p.to_string_lossy().contains("demo-host")));

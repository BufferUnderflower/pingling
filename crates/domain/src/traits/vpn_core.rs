...
//! The `service` layer, daemon binaries, IPC server, and `cli` binary all depend on
...
//! 2. Register it in the daemon launcher via `registry.register(descriptor, core)`
...
pub trait VpnCore: Send + Sync {
    ...
    /// `pingling_config_pipeline::StrategyPlan` bytes. Cores that don't

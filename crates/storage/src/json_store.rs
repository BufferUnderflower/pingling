...
impl JsonFileSettingsStorage {
    /// Create a store under the platform default layout for an app name.
    pub fn for_app(app_name: &str) -> Result<Self, VpnError> {
        Self::with_layout(&RuntimeLayout::for_app(app_name))
    }

    /// Create a store at the settings path from an explicit runtime layout.
    pub fn with_layout(layout: &RuntimeLayout) -> Result<Self, VpnError> {
        Self::with_path(layout.settings_file.clone())
    }
...
mod tests {
    ...
    #[test]
    #[serial]
    fn layout_path_uses_configured_settings_location() {
        ...
        let store = JsonFileSettingsStorage::with_layout(&layout).expect("store");
        assert_eq!(store.path(), layout.settings_file.as_path());
    }
}

...
//! runtime cache root (`pingling_util::paths::registry_dir()`), as
//! `<cache>/pingling/daemons/<pid>.json`. A sibling `latest.json` copy points
...
use pingling_util::paths::registry_dir as runtime_registry_dir;
use serde::Serialize;
use serde_json::json;
use std::fs;
use std::io;
...
pub struct DaemonAdvertisement {
    /// Always `"pingling"` — distinguishes our beacon from anything else
    ...
    /// `ws://host:port` for browser-compatible loopback JSON-RPC.
    /// `None` if the WebSocket listener failed to bind or was disabled.
...
impl DaemonAdvertisement {
    pub fn new(tcp: Option<String>, uds: Option<String>) -> Self {
        Self::with_websocket(tcp, uds, None)
    }
    ...
    pub fn with_websocket(
        ...
        Self {
            ...
            websocket,
            uds,
            log_file: std::env::var("PINGLING_DAEMON_LOG_FILE").ok(),
...
mod tests {
    ...
    fn registry_file_round_trip() {
        ...
        assert!(read.contains("\"service\":\"pingling\""));
        assert!(read.contains("\"tcp\":\"127.0.0.1:9999\""));
        assert!(!read.contains("\"websocket\""));
    ...
    fn beacon_replies_to_probe() {
        let ad = Arc::new(DaemonAdvertisement::with_websocket(
            Some("127.0.0.1:9000".into()),
            Some("/tmp/pingling-test.sock".into()),
            Some("ws://127.0.0.1:7879".into()),
        ));
        ...
        assert_eq!(result.service, "pingling");
        assert_eq!(result.hostname, "localhost");
        assert_eq!(result.tcp.as_deref(), Some("127.0.0.1:9000"));
        assert_eq!(result.websocket.as_deref(), Some("ws://127.0.0.1:7879"));

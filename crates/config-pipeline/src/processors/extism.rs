...
//!
//! Wraps an `Arc<dyn pingling_domain::Plugin>` so a loaded wasm plugin behaves
//! like a first-class [`ConfigProcessor`]. As of the slot-chain
//! migration, the adapter dispatches through the canonical
//! [`pingling_domain::slot_names::CONFIG_PROCESS`] slot — a middleware chain
...
//! ```
//!
//! The envelope is a [`pingling_domain::SlotContext`] carrying a typed payload:
//!
//! ```json
//! {
//!   "slot": "config.process",
...
//! ```
//!
//! The plugin returns a [`pingling_domain::SlotOutcome`] tagged enum:
//!
//! ```json
//! {"kind": "unchanged"}
...
//! pingling_domain::Plugin>` keeps the adapter generic: any implementation of
...
use pingling_domain::{run_slot_chain, slot_names, Plugin};
...
/// inside [`pingling_domain::SlotContext::payload`] and returned inside
/// [`pingling_domain::SlotOutcome::Continue`] / [`pingling_domain::SlotOutcome::Halt`].
///
...
/// adopt the slot chain return [`pingling_domain::SlotOutcome`] instead; this
...
pub struct ExtismProcessorAdapter {
    ...
    /// `name()` plus the method suffix, e.g. `"demo-plugin/config.process"`.
    name: String,
...
impl ConfigProcessor for ExtismProcessorAdapter {
    ...
    fn process(&self, config: Value, request: &ConfigRequest) -> Result<Value, String> {
        ...
        let invocation_id = pingling_domain::new_invocation_id();
...
mod tests {
    ...
    use pingling_domain::{Authenticator, VpnError};
    use std::sync::Mutex;

...
pub fn init(service_name: &str) -> io::Result<LogFiles> {
    ...
    unsafe {
        std::env::set_var("PINGLE_DAEMON_LOG_FILE", &log_files.latest_file);
    }
...
fn resolve_log_files(service_name: &str) -> LogFiles {
    let latest_file = util::paths::log_file();
    let directory = latest_file
        .parent()
        .map(Path::to_path_buf)
        .unwrap_or_else(|| std::env::temp_dir().join("pingle"));
...
mod tests {
    ...
    fn resolve_log_files_uses_runtime_cache_root() {
        ...
        assert_eq!(log_files.directory, util::paths::cache_root());
    }
}
